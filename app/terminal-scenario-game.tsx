    ...
    <main className="expansion-system-root terminal-runner">
      ...
      <div className="terminal-layout">
        ...
        <section className="terminal-window">
          ...
          <div className="terminal-history">{session.history.slice(session.screenHistoryStart).slice(-14).map((record, index) => <div key={`${record.line}-${index}`}><b>{record.cwd} $ {record.line}</b>{record.stdout && <pre>{record.stdout}</pre>}{record.stderr && <pre className="terminal-error">{record.stderr}</pre>}</div>)}{!session.history.length && <p>{lastOutput}</p>}</div>
