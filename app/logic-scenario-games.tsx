  ...
  return (
    <main className="logic-root">
      ...
      <header className="logic-topbar">
        ...
        <span className="logic-counter"><Braces size={16} /> 2 motorer · 6 sager</span>
      </header>
    ...
    <section className="logic-stage" tabIndex={0} onKeyDown={keyboard} aria-label="Fordelingsmatrix">
      ...
      <div className="logic-workspace">
        ...
        <div className="logic-grid-panel">
          <div className="logic-subject-tabs">{scenario.subjects.map((subject, index) => <button className={activeSubject === index ? "active" : ""} onClick={() => setActiveSubject(index)} key={subject.id}><b>{subject.label}</b><small>{subject.detail}</small></button>)}</div>
          ...
          <p className="logic-keyhint">Tast 1–3 for plads · piletaster for række · Enter for kontrol</p>
